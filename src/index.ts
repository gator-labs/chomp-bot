...
const replyWithPrimaryOptions = async (ctx: any) => {
  ...
  await kv.set(`state:${ctx?.from?.id}`, 'INIT');
...
bot.action('new.quickstart', async (ctx) => {
  const state = await kv.get(`state:${ctx.from.id}`);
  if (state === 'INIT') {
    ...
    if (questionDeck) {
      ...
      await kv.set(`state:${ctx.from.id}`, 'FIRST');
      if (imageUrl) {
        ctx.replyWithPhoto(
          { url: imageUrl },
...
bot.action(/^answering-first-order\.(.+)$/, async (ctx) => {
  ...
  if (state === 'FIRST') {
    ...
    await kv.set(`state:${ctx.from.id}`, 'SECOND');
...
bot.action('new.reveal', async (ctx) => {
  ...
  if (state === 'INIT') {
    ...
    if (!questionRevealCount) {
      ...
      await kv.set(`state:${ctx?.from?.id}`, 'REVEAL');
      ctx.reply(prompt, Markup.inlineKeyboard(buttons));
    }
  }
});
